...
function loadChunk(i){
  ...
  $('#instruction').textContent='Follow one complete idea: notice the Latin, rebuild the ordinary meaning, and only then name the transformation.';
  const firstLine=Number(p.cite.match(/4\.(\d+)/)[1]);
  $('#passage').innerHTML=c.lines.map(n=>`<span class="line"><span class="line-number">${firstLine+n}</span><span>${p.lines[n]}</span></span>`).join('');
  render();
}
...
function render(){
  ...
  const x=questions[q],stage=x.stage||1,stageNames={1:'Notice and unmask',2:'Rebuild the idea',3:'Name the transformation'};
  document.querySelectorAll('#learningPath span').forEach(s=>s.classList.toggle('active',+s.dataset.stage===stage));
  $('#question').innerHTML=`<div class="kind">Step ${stage}: ${stageNames[stage]} · ${x.type} · Question ${q+1} of ${questions.length}</div><fieldset><legend>${x.prompt}</legend>${x.choices.map((v,i)=>`<label class="choice"><input type="radio" name="answer" value="${i}"> ${v}</label>`).join('')}</fieldset><button class="action" id="check">Check my thinking</button><button class="hint" id="hint">Show one clue</button><div id="feedback" aria-live="polite"></div>`;
